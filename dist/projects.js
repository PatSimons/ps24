...
(() => {
      ...
      (context3) => {
        ...
        if (teasers.length > 0) {
          teasers.forEach((el) => {
            ...
            if (!isDesktop) {
              const teaserTitle2 = el.querySelector('[cs-el="teaserTitle"]');
              gsapWithCSS.set(teaserTitle2, { opacity: 0 });
              return;
            }
        ...
        const toolTip = document.querySelector('[cs-el="toolTip"]');
        ...
        if (!vertical) {
          gsapWithCSS.from(visibleProjectImgItems, {
            opacity: 0,
            x: "5rem",
            ease: "power3.out",
            delay: 0.35,
        ...
        function monitorCursorCollision(delay = 0.2) {
          ...
          document.addEventListener("mousemove", (event) => {
            ...
            if (isCursorOverElement(mouseX, mouseY, loopWrapper)) {
              ...
              gsapWithCSS.killTweensOf(toolTip, "autoAlpha");
              gsapWithCSS.to(toolTip, { autoAlpha: 0, duration: 0 });
            }
          });
        }
  ...
  window.Webflow ||= [];
  window.Webflow.push(() => {
    const projectImgItems = document.querySelectorAll('[cs-el="projectsTeaserWrap"]');
    ...
    const cmsFilter = document.querySelector('[cs-el="cmsFilter"]');
    if (cmsFilter) {
      gsapWithCSS.from(cmsFilter, { opacity: 0, delay: 3 });
      gsapWithCSS.set(cmsFilter, { autoAlpha: 1 });
      const cmsFilterLinks = cmsFilter.querySelectorAll('[cs-el="cmsFilterLink"]');
      const cmsItems = document.querySelectorAll('[cs-el="projectsTeaserWrap"]');
      cmsFilterLinks[0].classList.add("is-active");
      cmsFilterLinks.forEach((link) => {
        link.addEventListener("click", (e) => {
          e.preventDefault;
          cmsFilterLinks.forEach((link2) => {
            link2.classList.remove("is-active");
          });
          link.classList.add("is-active");
          const linkFilter = link?.querySelector("a").getAttribute("data-linkfilter");
          cmsItems.forEach((item) => {
            const cmsFilterItemsArray = item?.querySelectorAll(
            ...
            cmsFilterItemsArray.forEach((el) => {
              ...
              if (filtersArray.includes(linkFilter) || linkFilter === "all-projects") {
                ...
                gsapWithCSS.set(item, { autoAlpha: 0, x: "5rem" });
                gsapWithCSS.to(item, { autoAlpha: 1, x: 0, ease: "power3.out", duration: 1 });
              } else {
